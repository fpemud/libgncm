...
static struct nl_sock *_genl_sock (GncmLinuxPlatform *platform);
	...
	if (need_ext_data && obj->_link.ext_data == NULL) {
		...
		case NM_LINK_TYPE_WIFI:
			obj->_link.ext_data = (GObject *) nm_wifi_utils_new (ifi->ifi_index,
			                                                     _genl_sock (GNCM_LINUX_PLATFORM (platform)),
			                                                     TRUE);
			break;
		case NM_LINK_TYPE_OLPC_MESH:
#if HAVE_WEXT
...
} GncmLinuxPlatformPrivate;

struct _GncmLinuxPlatform {
	NMPlatform parent;
	GncmLinuxPlatformPrivate _priv;
};

struct _GncmLinuxPlatformClass {
	NMPlatformClass parent;
};

G_DEFINE_TYPE (GncmLinuxPlatform, nm_linux_platform, NM_TYPE_PLATFORM)

#define GNCM_LINUX_PLATFORM_GET_PRIVATE(self) _BYX_GET_PRIVATE (self, GncmLinuxPlatform, GNCM_IS_LINUX_PLATFORM, NMPlatform)

NMPlatform *
gncm_linux_platform_new (gboolean log_with_ptr, gboolean netns_support)
{
	...
	return g_object_new (GNCM_TYPE_LINUX_PLATFORM,
...
void
gncm_linux_platform_setup (void)
{
	nm_platform_setup (gncm_linux_platform_new (FALSE, FALSE));
}

/*****************************************************************************/

static struct nl_sock *
_genl_sock (GncmLinuxPlatform *platform)
{
	GncmLinuxPlatformPrivate *priv = GNCM_LINUX_PLATFORM_GET_PRIVATE (platform);
...
_byx_logging_clear_platform_logging_cache_impl (void)
{
	while (sysctl_clear_cache_list) {
		GncmLinuxPlatformPrivate *priv = GNCM_LINUX_PLATFORM_GET_PRIVATE (sysctl_clear_cache_list->data);
                      ...
                      NMPlatformKernelSupportFlags request_flags)
{
	...
	nm_assert (GNCM_IS_LINUX_PLATFORM (platform));
...
static guint32
_nlh_seq_next_get (GncmLinuxPlatformPrivate *priv)
{
...

	{
		...
		case RTM_NEWROUTE: {
			...
			if (is_ipv6 || NM_FLAGS_HAS (obj->ip_route.r_rtm_flags, RTM_F_CLONED)) {
				...
				priv = GNCM_LINUX_PLATFORM_GET_PRIVATE (platform);
				if (NM_FLAGS_HAS (priv->delayed_action.flags, DELAYED_ACTION_TYPE_WAIT_FOR_NL_RESPONSE)) {
					guint i;
...
static void
gncm_linux_platform_init (GncmLinuxPlatform *self)
{
	GncmLinuxPlatformPrivate *priv = GNCM_LINUX_PLATFORM_GET_PRIVATE (self);
...
constructed (GObject *_object)
{
	...
	G_OBJECT_CLASS (gncm_linux_platform_parent_class)->constructed (_object);
...
dispose (GObject *object)
{
	...
	G_OBJECT_CLASS (gncm_linux_platform_parent_class)->dispose (object);
}

static void
finalize (GObject *object)
{
	GncmLinuxPlatformPrivate *priv = GNCM_LINUX_PLATFORM_GET_PRIVATE (object);
	...
	G_OBJECT_CLASS (gncm_linux_platform_parent_class)->finalize (object);
}

static void
gncm_linux_platform_class_init (GncmLinuxPlatformClass *klass)
{
